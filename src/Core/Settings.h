...
#define COMMON_SETTINGS(M) \
    ...
    M(UInt64, empty_result_for_aggregation_by_empty_set, 0, "0 - Return one row with default value when aggregating without keys on empty set. 1 - Return empty result when aggregating without keys on empty set. 2 - Return one row with default value when aggregating on empty set.", 0) \
