...
public:
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr &, size_t input_rows_count) const override
    {
        ...
        if (!src_col)
            throw Exception(
                "Illegal column " + arguments[0].name + " in argument of function 'arrayRoundCarry'", ErrorCodes::ILLEGAL_COLUMN);

        const ColumnArray * target_col = checkAndGetColumn<ColumnArray>(arguments[1].column.get());

        if (!target_col)
            throw Exception(
                "Illegal column " + arguments[1].name + " in argument of function 'arrayRoundCarry'", ErrorCodes::ILLEGAL_COLUMN);
