    ...
    if (static_cast<int32_t>(digits) + exponent > static_cast<int32_t>(precision - scale))
    {
        if constexpr (throw_exception)
        {
            ...
            if constexpr (is_big_int_v<typename T::NativeType>)
                throw ParsingException(ErrorCodes::ARGUMENT_OUT_OF_BOUND, pattern, digits, x.value, exponent, scale, precision);
            else
                throw ParsingException(ErrorCodes::ARGUMENT_OUT_OF_BOUND, pattern, digits, x, exponent, scale, precision);
        }
        else
            return ReturnType(false);
    }
