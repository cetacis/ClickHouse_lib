...
rs*)
    function configure() {
        cmake ../src -DCLICKHOUSE_SPLIT_BINARY=0 -DENABLE_REPLXX=1 -DENABLE_READLINE=1 -DUSE_DEBUG_HELPERS=1 -DCMAKE_BUILD_TYPE=Release -DENABLE_LIBRARIES=0 -DUSE_UNWIND=1 -DENABLE_TESTS=0 -DENABLE_UTILS=0 -DCMAKE_EXE_LINKER_FLAGS="-s -Wl,--dynamic-linker,$ld" $compiler
    }
    ;;
rel*)
    function configure() {
